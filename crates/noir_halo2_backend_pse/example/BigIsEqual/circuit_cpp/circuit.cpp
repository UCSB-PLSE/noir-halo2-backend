...
Fr_neq(&expaux[0],&signalValues[mySignalStart + 1],&circuitConstants[0]); // line circom 20
if(Fr_isTrue(&expaux[0])){
{
PFrElement aux_dest = &signalValues[mySignalStart + 2];
// load src
Fr_div(&expaux[0],&circuitConstants[1],&signalValues[mySignalStart + 1]); // line circom 20
// end load src
...
// load src
Fr_neg(&expaux[2],&signalValues[mySignalStart + 1]); // line circom 22
Fr_mul(&expaux[1],&expaux[2],&signalValues[mySignalStart + 2]); // line circom 22
Fr_add(&expaux[0],&expaux[1],&circuitConstants[1]); // line circom 22
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
Fr_mul(&expaux[1],&signalValues[mySignalStart + 1],&signalValues[mySignalStart + 0]); // line circom 23
{{
...
// load src
Fr_sub(&expaux[0],&signalValues[mySignalStart + 2],&signalValues[mySignalStart + 1]); // line circom 9
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
...
cmp_index_ref_load = ((1 * Fr_toInt(&lvar[2])) + 0);
Fr_add(&expaux[0],&lvar[1],&ctx->signalValues[ctx->componentMemory[mySubcomponents[((1 * Fr_toInt(&lvar[2])) + 0)]].signalStart + 0]); // line circom 35
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
BigIsEqual_2_create(1,0,ctx,"main",0);
BigIsEqual_2_run(0,ctx);
}
