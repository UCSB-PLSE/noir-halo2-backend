...
NULL };
uint get_main_input_signal_start() {return 4098;}

uint get_main_input_signal_no() {return 1;}

uint get_total_signal_no() {return 4099;}

uint get_number_of_components() {return 1;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 4099;}
...
{{
Fr_eq(&expaux[0],&signalValues[mySignalStart + 4097],&lvar[2]); // line circom 10
}}
...
{
Fr_sub(&expaux[2],&signalValues[mySignalStart + 4097],&lvar[2]); // line circom 11
...
PFrElement aux_dest = &lvar[1];
// load src
Fr_add(&expaux[0],&lvar[1],&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[2])) + 0)]); // line circom 12
// end load src
...
{
PFrElement aux_dest = &signalValues[mySignalStart + 4096];
// load src
// end load src
Fr_copy(aux_dest,&lvar[1]);
}
{
Fr_sub(&expaux[2],&signalValues[mySignalStart + 4096],&circuitConstants[2]); // line circom 16
Fr_mul(&expaux[1],&signalValues[mySignalStart + 4096],&expaux[2]); // line circom 16
{{
...
Decoder_0_create(1,0,ctx,"main",0);
Decoder_0_run(0,ctx);
}
